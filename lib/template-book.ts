...
.composition-story-scene{position:relative;isolation:isolate}
.composition-story-scene .art{position:absolute;inset:0;width:100%;height:100%;padding:0;z-index:0}
.composition-story-scene .art>div{height:100%}
.composition-story-scene .art img{width:100%;height:100%;object-fit:cover}
.composition-story-scene .copy{position:relative;z-index:1;width:36%;height:56%;margin:4.8% 0 0 7%;padding:2%;background:#fff9e9ed;color:#344b43;justify-content:center;border:0;box-shadow:none}
.composition-story-scene .copy h2{margin-bottom:4%;font-style:italic}
.composition-story-scene .meaning{border:0;padding:0;margin-top:4%;font-size:12pt}
.composition-story-scene .original{line-height:1.5}
.composition-story-scene .folio{z-index:2;background:#fff9e9e8;color:#344b43;padding:.3%}
`; }
...
export function templateDesign(id: TemplateId) {
    ...
    const geometry: Record<Layout, string> = {
        ...
        'story-scene': 'Full 420:250 spread. Continuous landscape in the bottom 28 percent, expressive narrative scene on the right 48 percent. Reserve x=7–43%, y=8–64% as pale low-detail sky for editable text. Keep subjects outside this text-safe rectangle. No separate bottom reading band, no baked-in lettering, no photographed book mockup.',
    };
