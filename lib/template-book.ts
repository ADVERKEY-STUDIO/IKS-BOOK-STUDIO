...
export function parseTemplateBook(input: unknown, projectId?: string): TemplateBook {
    ...
    if (b.templateRevision !== undefined && b.templateRevision !== TEMPLATE_REVISION) throw Error('Unsupported template revision. Keep this ZIP and update Book Studio before importing it.');
...
export function renderTemplateBook(book: TemplateBook, urls: Record<string, string> = {}, font = 'fonts/book-sanskrit.ttf') {
    ...
    const { height } = templateSize(book.templateId);
...
export function renderTemplatePage(book:TemplateBook,index:number,urls:Record<string,string>={},font='/fonts/book-sanskrit.ttf') {
