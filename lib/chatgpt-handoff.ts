...
${blueprintPrompt(templateBlueprints(draft.templateId).find(b => b.id === 'reference-beanstalk-diagonal')?.id || templateBlueprints(draft.templateId)[0].id)}
...
export function chapterPrompt(draft: Draft) {
  ...
  return sourceBookPrompt(draft)
    .replace('Generate an actual character reference image first if figures recur. Use it for every scene.', 'Use the attached approved character-reference.png and style-sample.png for every scene. Preserve identity; if their rendering conflicts with the selected template, correct the rendering to the template before continuing.')
    .replace(/CONSISTENCY: First create[^\n]+/, 'CONSISTENCY: The attached approved/character-reference.png and approved/style-sample.png are the approved identity and style references. Reuse both as actual image inputs for EVERY scene. Preserve faces, proportions, outfits, brushwork, palette and lighting. If this sheet conflicts with the selected template, correct its rendering first while preserving source identity. Compare each result against these references and correct visible drift. Include the approved character sheet as images/character-reference.png in the returned ZIP, but not as a story page.')
