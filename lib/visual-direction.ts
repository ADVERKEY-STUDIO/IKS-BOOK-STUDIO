...
export type VisualDirection = { notebookMode?: 'summary' | 'full'; notebookPageCount?: number; audience: string; characters: string; notes: string; handoff?: 'references' | 'template' | 'sample' | 'approved'; feedback?: string };
export function parseVisualDirection(value: unknown): VisualDirection | undefined {
  ...
  return { ...(notebookPageCount===undefined?{}:{notebookPageCount}), ...(data.notebookMode ? { notebookMode: data.notebookMode as VisualDirection['notebookMode'] } : {}), audience: data.audience as string, characters: data.characters as string, notes: data.notes as string,
