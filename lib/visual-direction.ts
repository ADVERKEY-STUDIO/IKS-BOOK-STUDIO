...
export type VisualDirection = { notebookMode?: 'summary' | 'full'; audience: string; characters: string; notes: string; handoff?: 'references' | 'template' | 'sample' | 'approved'; feedback?: string };
export function parseVisualDirection(value: unknown): VisualDirection | undefined {
  ...
  return { ...(data.notebookMode ? { notebookMode: data.notebookMode as VisualDirection['notebookMode'] } : {}), audience: data.audience as string, characters: data.characters as string, notes: data.notes as string,
...
export function sourceBookPrompt(draft: RequestDraft) {
  ...
  const base = draft.templateId === 'iks-notes' ? notebookBookPrompt(draft.id, draft.title, draft.source?.name || 'source.pdf', draft.language, direction?.notebookMode || 'full') : bookPrompt(draft.id, draft.templateId, draft.title, draft.source?.name || 'source.pdf', draft.language);
