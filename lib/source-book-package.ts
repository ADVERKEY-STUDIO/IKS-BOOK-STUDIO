...
export function attachSourceBookManifest(result:ExternalManuscriptResult,manifest:SourceBookManifest):ExternalManuscriptResult {
  ...
  const missing = manifest.verses.flatMap(verse => {
    const target = sections[verse.sectionNumber - 1];
    if (!target) return [`Verse ${verse.id}: section ${verse.sectionNumber} is not in this manuscript.`];
    const blocks = target.html.split(`data-source-verse-block="${verse.id}"`).length - 1;
    const markers = target.html.split(`{{SLOKA:${verse.id}}}`).length - 1;
    return blocks + markers === 1 ? [] : [`Verse ${verse.id}: ${blocks + markers ? 'duplicate blocks' : 'missing block'} in section ${verse.sectionNumber} (${target.title}). Place one :::sloka ${verse.id} block in this section.`];
  });
...
Before delivering the ZIP, reopen its actual chapter files and check every manifest verse ID: exactly one matching block must exist in its declared section, with identical Sanskrit. Check every image anchor against the actual section text. Do not deliver a manifest describing verses that were omitted from the packaged Markdown. Include just one manuscript package and one source-manifest.json; omit duplicate drafts and operating-system metadata.
