...
try{
 ...
 await page.locator('dialog[open] .ts-template-layout-samples select').selectOption('reference-beanstalk-diagonal');
 ...
 await page.getByRole('button',{name:'Move text',exact:true}).click();
 const textBox=page.getByRole('button',{name:'Move original text box 1',exact:true});
 const initial=await textBox.boundingBox();
 await page.mouse.move(initial.x+initial.width/2,initial.y+initial.height/2);await page.mouse.down();await page.mouse.move(initial.x+initial.width/2+45,initial.y+initial.height/2-25,{steps:8});await page.mouse.up();
 await page.waitForTimeout(500);
 const moved=await textBox.boundingBox();assert.ok(moved.x>initial.x+30,'drag moves text');
 await textBox.focus();await page.keyboard.press('ArrowRight');await page.waitForTimeout(300);
 const resizedHandle=textBox.locator('[data-resize]');const handle=await resizedHandle.boundingBox();
 await page.mouse.move(handle.x+7,handle.y+7);await page.mouse.down();await page.mouse.move(handle.x+37,handle.y+17,{steps:5});await page.mouse.up();await page.waitForTimeout(500);
 const resized=await textBox.boundingBox();assert.ok(resized.width>moved.width+20,'resize changes box');
 await page.reload();await page.getByRole('button',{name:'Move text',exact:true}).click();
 const restored=await page.getByRole('button',{name:'Move original text box 1',exact:true}).boundingBox();assert.ok(Math.abs(restored.width-resized.width)<2,'positions survive reload');
 await page.getByRole('button',{name:'Reset text positions',exact:true}).click();
 await page.getByRole('button',{name:'Done placing text',exact:true}).click();
 ...
 await page.locator('.ts-layout-grid select').first().selectOption('reference-beanstalk-landscape');
 ...
 const rebuilt=parseTemplateBook(JSON.parse(new TextDecoder().decode(archive['book.json'])));assert.equal(rebuilt.pages[0].blueprint,'reference-beanstalk-landscape');assert.notEqual(rebuilt.projectId,planned.projectId);
