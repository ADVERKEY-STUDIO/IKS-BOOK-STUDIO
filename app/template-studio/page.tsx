...
import { templatePrintSize, blueprintFor, templateBlueprints, layoutIssues, imageGeometryIssue, inspectRenderedBook, blueprintPrompt } from '../../lib/template-layouts';
...
import { templates, selectableTemplates, templateLayout, templateAppearanceCss, parseTemplateBook, importTemplateManuscript, continuationPrompt, renderTemplateBook, assetName, type TemplateId, type TemplateBook, type BookPage } from '../../lib/template-book';
...
export default function TemplateStudio() {
    ...
    function editPage(p: Partial<BookPage>) { if (book)
        patch({ book: { ...book, pages: book.pages.map((v, i) => i === selected ? { ...v, ...p, ...(p.blueprint && p.blueprint!==v.blueprint && draft?.images[v.image] ? {artworkBlueprint:v.artworkBlueprint || v.blueprint} : {}) } : v) } }); }
    ...
    async function downloadPdf() {
        ...
        const fitIssues=[...layoutIssues(book).filter(note=>/artwork belongs/.test(note)),...inspectRenderedBook(doc).filter(note=>/overflow|did not load/.test(note))];
        ...
        try {
            ...
            const sizeAt=(i:number)=>templatePrintSize(book.templateId,spreads[i].classList.contains('cover'));
            ...
            for (let i = 0; i < spreads.length; i++) {
                ...
                const canvas = await html2canvas(spreads[i], { scale: 1.5, backgroundColor: null, logging: false, windowWidth: 1800, windowHeight: 1200, onclone: cloned => { const originals=doc.querySelectorAll<HTMLElement>('.planned-text,.story-text'); cloned.querySelectorAll<HTMLElement>('.planned-text,.story-text').forEach((el,index)=>{if(originals[index])el.style.fontSize=doc.defaultView!.getComputedStyle(originals[index]).fontSize;}); } });
                const size=sizeAt(i);
