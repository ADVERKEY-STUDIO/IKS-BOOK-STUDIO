...
export default function TemplateStudio() {
    ...
    return <main className="ts"><fieldset disabled={busy} style={{ border: 0, padding: 0, margin: 0, minWidth: 0 }}><style>{templateAppearanceCss()}</style><header className="ts-header"><Link href="/">← Book Studio</Link><strong>Books, made your way.</strong><span role="status">{busy ? 'Working…' : notice}</span></header><AccountPanel onSession={setEmail}/><div className="ts-actions">{draft && <button disabled={busy} onClick={() => void run(saveNow)}>Save book{email ? ' to account' : ' now'}</button>}<button disabled={busy} onClick={() => setStep(0)}>My saved books</button></div>{email && <section className="ts-panel"><h2>Your cloud library</h2><p>Saved to {email}. Sign in with this email in another browser to open these books.</p><button disabled={busy} onClick={() => void run(refreshCloud)}>Refresh cloud books</button>{cloudBooks.length === 0 && <p>No cloud books yet. Open a browser book and choose Save book to account.</p>}{cloudBooks.map(item => <button className="ts-saved" disabled={busy} key={item.id} onClick={() => void run(async () => { await writeQueue.current; if (draft) await storage('write', draft); const restored = await downloadDraft(item, email); const existing = saved.find(d => d.id === restored.id); if (existing) { const backupId = crypto.randomUUID(); await storage('write', { ...existing, id: backupId, book: existing.book ? { ...existing.book, projectId: backupId, title: existing.title + ' (local backup)' } : undefined, title: existing.title + ' (local backup)', cloud: undefined }); } await storage('write', restored); setSaved(await storage('read')); setDraft(restored); setStep(restored.book ? 2 : 1); setSelected(0); setUnmatched([]); })}>{item.title} · Open cloud copy</button>)}</section>}<nav aria-label="Book creation steps">{['Choose a template', 'Source & prompt', 'Assemble & edit'].map((label, i) => <button key={label} disabled={busy || !draft && i > 0 || i === 2 && !book} aria-current={step === i ? 'step' : undefined} onClick={() => setStep(i)}>{i + 1}. {label}</button>)}</nav>{error && <p className="ts-error" role="alert">{error}</p>}
