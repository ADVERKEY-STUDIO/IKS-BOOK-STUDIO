...
export function LayoutDiagram({templateId,blueprint}:{templateId:string;blueprint:string}){
 const b=[...templateBlueprints(templateId),...(templateId==='iks-notes'?legacyNotebookLayouts:[])].find(v=>v.id===blueprint)!;
...
export default function LayoutReview({book,busy,onClose,onCreate}:{book:TemplateBook;busy:boolean;onClose:()=>void;onCreate:(book:TemplateBook)=>Promise<void>}){
 ...
 return <section className="ts-panel ts-layout-review" aria-label="Review spread layout plan">
  ...
  <div className="ts-layout-grid">{planned.pages.map((p,i)=><article key={p.id}>
   ...
   <strong>{i+1}. {p.title}</strong><label>Spread arrangement<select value={p.blueprint} onChange={e=>setPlanned({...planned,pages:planned.pages.map((v,j)=>j===i?{...v,blueprint:e.target.value,layoutReason:'Selected during whole-book layout review.'}:v)})}>{options.filter(b=>!p.noteSections||b.original.length===p.noteSections.length).map(b=><option key={b.id} value={b.id}>{b.name}</option>)}</select></label>
