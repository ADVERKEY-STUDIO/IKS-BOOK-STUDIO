...
export default function ChildrenGallery({onChoose,disabled}:{onChoose:(id:TemplateId)=>void;disabled:boolean}){
 ...
 <button className="children-reference" onClick={()=>{setPage(0);setOpened(t);}} aria-label={`Look inside ${t.reference}`}><img src={t.demo} alt={`${t.reference} — original book reference`} loading="lazy"/><span>{'previewLabel' in t ? t.previewLabel : `${t.images.length} previews`} · View original book ↗</span></button>
 <div className="ts-literary-info"><small>{t.creator}</small><h3>{t.reference}</h3><p>{t.description}</p><div className="ts-literary-actions"><button onClick={()=>{setPage(0);setOpened(t);}}>Look inside</button><button disabled={disabled} onClick={()=>onChoose(t.id)}>Use template</button></div></div>
 </article>)}</div>
 ...
 <div className="children-preview"><img src={opened.images[page]} alt={`${opened.reference} original preview ${page+1}`}/></div>
 ...
 <style>{`.children-gallery{scroll-margin-top:30px}.children-eyebrow{font:11px Arial,sans-serif;letter-spacing:.16em;color:#8e5033;margin-top:35px}.children-reference{display:block!important;width:100%;border:0!important;border-radius:0!important;padding:24px!important;background:#eeeade!important;cursor:pointer;color:#234139!important;text-align:left!important}.children-reference img{display:block;width:100%;height:280px;object-fit:contain}.children-reference span{display:block;font-size:11px;margin-top:18px}.children-preview{background:#f1eee6;padding:24px;text-align:center}.children-preview img{max-width:100%;height:55vh;object-fit:contain}.children-preview-actions{display:flex;justify-content:center;gap:24px;align-items:center;padding:15px}.children-preview-footer{padding:20px 28px;display:flex;align-items:center;gap:24px;justify-content:space-between;border-top:1px solid #ddd}.children-preview-footer p{max-width:650px;font-size:13px}.children-preview-footer a{font-size:13px}.children-preview-footer>button{flex-shrink:0;background:#234139;color:white}@media(max-width:650px){.children-preview-footer{display:block}.children-reference img{height:240px}.children-preview img{height:40vh}}`}</style>
 </section>
}
