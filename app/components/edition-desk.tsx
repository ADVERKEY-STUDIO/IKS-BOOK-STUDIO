...
type Props = { edition: Edition; onAction: (action: EditionAction) => Promise<void>; onUpload: (file: File) => Promise<void>; onDownload: (key: string, name: string) => Promise<void>; onExport: () => Promise<string>; onBack: () => void; onReload: () => Promise<void> };
export function EditionDesk({ edition, onAction, onUpload, onDownload, onExport, onBack, onReload }: Props) {
  const [tab, setTab] = useState<'setup' | 'source' | 'proof'>(edition.passages.length ? 'source' : 'setup');
  ...
  async function run(work: () => Promise<void>, success: string) {
    ...
    try { await work(); setMessage(success); } catch (e) { setError(e instanceof Error ? e.message : 'Could not save. Your changes remain here.'); } finally { setBusy(false); }
  }
  ...
  return <main className="edition-desk">
    ...
    <nav aria-label="Edition workspace">{(['setup', 'source', 'proof'] as const).map(t => <button key={t} disabled={busy || dirty} aria-current={tab === t ? 'page' : undefined} onClick={() => setTab(t)}>{t === 'setup' ? 'Edition setup' : t === 'source' ? 'Source desk' : 'Reading proof'}</button>)}</nav>
