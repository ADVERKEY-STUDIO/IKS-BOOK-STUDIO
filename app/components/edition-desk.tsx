"use client";
import { BookReleaseWorkspace } from "./book-release-workspace";
import { BookReviewWorkspace } from "./book-review-workspace";
...
type Props = { onSnapshot:(final:boolean)=>Promise<Edition>; onBackup:()=>Promise<Blob>; onRestore:(file:File)=>Promise<void>; onArtUpload:(input:ArtUpload)=>Promise<void>; onArtPackage:(id:string)=>Promise<void>; onReferenceUpload: (input: ReferenceUpload) => Promise<void>; onReferenceImage: (key: string) => Promise<Blob>; onReferencePackage: (id: string) => Promise<void>; inspiration?: BookInspiration; onBrief: () => Promise<string>; edition: Edition; onAction: (action: EditionAction) => Promise<void>; onUpload: (file: File) => Promise<void>; onDownload: (key: string, name: string) => Promise<void>; onExport: () => Promise<string>; onBack: () => void; onReload: () => Promise<void> };
export function EditionDesk({ onSnapshot, onBackup, onRestore, onArtUpload, onArtPackage, onReferenceUpload, onReferenceImage, onReferencePackage, inspiration, onBrief, edition, onAction, onUpload, onDownload, onExport, onBack, onReload }: Props) {
  const [tab, setTab] = useState<'setup' | 'source' | 'art' | 'references' | 'storyboard' | 'designer' | 'production' | 'book' | 'review' | 'proof' | 'release'>(edition.passages.length ? 'source' : 'setup');
  const [reviewStoryboardPlan,setReviewStoryboardPlan] = useState('');
  const [designLayer,setDesignLayer] = useState('');
  const [sourceReviewField,setSourceReviewField] = useState<PassageField>('original');
  ...
  return <main className="edition-desk">
    ...
    </section><div className="edition-source-grid"><aside className="edition-panel"><h2>{passages.length} active passages</h2>{passages.map((p,i) => <button key={p.id} disabled={busy || passageDirty} aria-current={passage?.id === p.id ? 'true' : undefined} onClick={() => setSelected(p.id)}><strong>Passage {i+1}</strong><small>{p.fields.original.status} · revision {p.revision}</small><span>{p.fields.original.text.slice(0,65)}</span></button>)}</aside>{passage ? <PassageEditor key={`${passage.id}-${passage.revision}-${sourceReviewField}`} initialField={sourceReviewField} passage={passage} onDirty={setPassageDirty} next={passages[passages.indexOf(passage)+1]} busy={busy} onAction={async action => { await run(() => onAction(action), 'Passage revision saved.'); }} /> : <section className="edition-panel"><h2>Your source desk is ready</h2><p>Import a source or add your first passage to begin.</p></section>}</div>
    ...
    {tab === 'art' && <ArtDirectionWorkspace edition={edition} inspiration={inspiration} busy={busy} onDirty={setPassageDirty} onBrief={onBrief} onAction={action => run(() => onAction(action), action.type === 'approve-art-guide' ? 'Art guide approved for production.' : 'Art guide version saved.')} />}
    ...
    {tab === 'storyboard' && <StoryboardWorkspace initialPlanId={reviewStoryboardPlan} edition={edition} busy={busy} onDirty={setPassageDirty} onAction={action => run(() => onAction(action), 'Storyboard saved.')} />}
    ...
    {tab === 'book' && <BookProductionWorkspace edition={edition} busy={busy} onDirty={setPassageDirty} onImage={onReferenceImage} onAction={action=>run(()=>onAction(action),'Book production saved.')} onDesign={id=>{setDesignLayer('');setDesignPlan(id);setTab('designer');}} />}
    {tab === 'release' && <BookReleaseWorkspace edition={edition} onSnapshot={onSnapshot} onBackup={onBackup} onRestore={onRestore} onImage={onReferenceImage} onReview={()=>setTab('review')} />}
    {tab === 'review' && <BookReviewWorkspace edition={edition} busy={busy} onDirty={setPassageDirty} onImage={onReferenceImage} onAction={action=>run(()=>onAction(action),'Review record saved.')} onTarget={target=>{if(target.workspace==='art'){setTab('art');}else if(target.workspace==='storyboard'){setReviewStoryboardPlan(target.planId||'');setTab('storyboard');}else if(target.passageId){setSelected(target.passageId);setSourceReviewField(target.field||'original');setTab('source');}else if(target.planId){setDesignPlan(target.planId);setDesignLayer(target.layerId||'');setTab('designer');}}} />}
    {tab === 'designer' && <SpreadDesigner initialLayerId={designLayer} onReview={()=>setTab('review')} initialPlanId={designPlan} edition={edition} busy={busy} onDirty={setPassageDirty} onImage={onReferenceImage} onAction={action => run(() => onAction(action), 'Composition saved.')} />}
...
function PassageEditor({ initialField='original', passage: p, next, busy, onAction, onDirty }: { initialField?:PassageField; onDirty: (dirty: boolean) => void; passage: Passage; next?: Passage; busy: boolean; onAction: (action: EditionAction) => Promise<void> }) {
  const [field, setField] = useState<PassageField>(initialField);
  const [text, setText] = useState(p.fields[initialField].text);
  const [origin, setOrigin] = useState<Provenance>(p.fields[initialField].provenance);
