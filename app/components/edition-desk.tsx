...
type Props = { inspiration?: BookInspiration; onInspiration: () => void; onBrief: () => Promise<string>; edition: Edition; onAction: (action: EditionAction) => Promise<void>; onUpload: (file: File) => Promise<void>; onDownload: (key: string, name: string) => Promise<void>; onExport: () => Promise<string>; onBack: () => void; onReload: () => Promise<void> };
export function EditionDesk({ inspiration, onInspiration, onBrief, edition, onAction, onUpload, onDownload, onExport, onBack, onReload }: Props) {
  const [tab, setTab] = useState<'setup' | 'source' | 'art' | 'proof'>(edition.passages.length ? 'source' : 'setup');
  ...
  return <main className="edition-desk">
    ...
    <nav aria-label="Edition workspace">{(['setup', 'source', 'art', 'proof'] as const).map(t => <button key={t} disabled={busy || dirty} aria-current={tab === t ? 'page' : undefined} onClick={() => setTab(t)}>{t === 'setup' ? 'Edition setup' : t === 'source' ? 'Source desk' : t === 'art' ? 'Art direction' : 'Reading proof'}</button>)}</nav>
