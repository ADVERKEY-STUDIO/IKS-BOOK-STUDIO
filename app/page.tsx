...
export default function Home() {
  ...
  async function prepareDraft(scope: "sample" | "all" | "active" | "thin", options: { repairOnly?: boolean; chapterId?: number } = {}) {
    ...
    if (!options.repairOnly && !chapterOnePassed && requestedChapterIds.some((chapterId) => chapterId !== 1)) {
      notify("Phase 8 safety gate: generating Chapter 1 only. Chapters 2–6 stay untouched until its measurements pass.");
    }
...
function Phase7Report({ evaluation }: { evaluation?: ChapterOneGate }) {
  if (!evaluation) return <div className="phase7-report waiting"><b>PHASE 8 · CHAPTER 1 GATE</b><span>Waiting for one measured 500–900 word Chapter 1 generation. Chapters 2–6 are locked.</span></div>;
