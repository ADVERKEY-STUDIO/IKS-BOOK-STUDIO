...
type DesignerPageRevision = {
  ...
  columnGap: number;
  pagePadding: number;
  borderStyle: "inherit" | "none" | "solid" | "double" | "dashed";
  borderColor: string;
  borderWidth: number;
  borderInset: number;
  borderRadius: number;
...
type Project = {
  ...
  designerLayoutSnapshot?: boolean;
...
type PdfExportMode = "draft" | "publication";
const DRAFT_PROOF_LABEL = "DRAFT PROOF";

function publicationStatus(project: Project) {
  const result = assessPublication(project.chapters);
  const coverSlot = project.externalIllustrations?.slots.find((slot) => slot.role === "cover");
  if (!coverSlot || (coverSlot.status === "ready" && coverSlot.imageUrl)) return result;
  return {
    ready: false,
    blockers: [...result.blockers, {
      code: "missing-illustration" as const,
      chapterId: 0,
      chapterTitle: "Front cover",
      message: "The front cover still needs its approved illustration.",
    }],
  };
}

function coverSubtitleFor(project: Project) {
  const illustrated = project.chapters.filter((chapter) => chapterRequiresIllustration(chapter) && Boolean(chapter.imageUrl || chapter.importedPages?.some((page) => page.imageUrl))).length;
  const planned = project.chapters.filter(chapterRequiresIllustration).length;
  return planned > 0 && illustrated === planned
    ? "An illustrated journey through Indian knowledge and ideas"
    : "A draft awaiting its planned illustrations";
}

type SelectablePdf = {
  setFontSize: (size: number) => SelectablePdf;
...
function addSelectableTextLayer(pdf: SelectablePdf, sheet: HTMLElement, widthMm = 210, heightMm = 297) {
  ...
  while (node) {
    ...
    if (text && parent) {
      ...
      if (style.display !== "none" && style.visibility !== "hidden" && rect.width > 0 && rect.height > 0) {
        const x = Math.max(0, (rect.left - sheetRect.left) / sheetRect.width * widthMm);
        const y = Math.max(0, (rect.bottom - sheetRect.top) / sheetRect.height * heightMm);
        const fontSize = Math.max(5, Math.min(30, Number.parseFloat(style.fontSize || "12") * .75));
        pdf.setFontSize(fontSize).text(text, x, y, { maxWidth: Math.max(2, rect.width / sheetRect.width * widthMm), renderingMode: "invisible" });
        added += 1;
      }
    }
  ...
  if (!added) {
    ...
    if (fallback) pdf.setFontSize(8).text(fallback, widthMm * .057, heightMm * .061, { maxWidth: widthMm * .886, renderingMode: "invisible" });
  }
}
...
const emptyProject: Project = {
  ...
  bookFormat: DEFAULT_NEW_BOOK_FORMAT,
...
function normalizeProject(saved: Project): Project {
  ...
  // Theme inference must not replace a saved border choice or invent one.
  const border = bookBorder(cleanSaved.bookBorder || "No Border");
  ...
  const childFirstSaved: Project = {
    ...
    bookFormat: isBookFormatId(cleanSaved.bookFormat) ? cleanSaved.bookFormat : LEGACY_BOOK_FORMAT,
  ...
  const normalizedChapters = (cleanSaved.chapters ?? []).map((chapter, index) => {
    const title = readerFacingChapterTitle(chapter.title || `Chapter ${index + 1}`);
    return {
...
function isDesignerReady(project: Project) {
  return Boolean(project.adaptationPlanConfirmed && project.chapters.length && project.chapters.every(isPublishApproved));
}
...
export default function Home() {
  ...
  const [pdfExportMode, setPdfExportMode] = useState<PdfExportMode>("draft");
  ...
  async function editionRequest(action?: EditionAction, file?: File, reference?: ReferenceUpload, art?: ArtUpload) {
    let response: Response;
    if (file) { const form = new FormData(); form.set("projectId", project.id); form.set("expectedRevision", String(project.edition?.revision)); form.set("file", file); if (reference) { form.set("referenceId", reference.referenceId); form.set("view", reference.view); form.set("caption", reference.caption); form.set("credit", reference.credit); form.set("provenance", reference.provenance); } if(art){form.set("requestId",art.requestId);form.set("note",art.note);form.set("caption",art.caption);form.set("credit",art.credit);form.set("provenance",art.provenance);} response = await fetch(art ? "/api/edition/art-image" : reference ? "/api/edition/reference-image" : "/api/edition/source", { method: "POST", headers: ownerHeaders(), body: form }); }
  ...
  async function acceptExternalManuscript(result: ExternalManuscriptResult, fileName: string) {
    const importedChapters: Chapter[] = result.sections.map((section, index) => {
      ...
      const visibleTitle = readerFacingChapterTitle(section.title || `${section.kind[0].toUpperCase()}${section.kind.slice(1)}`);
      const needsIllustration = !/^(?:glossary|appendix)$/i.test(section.kind);
      return {
        id: index + 1,
        title: visibleTitle,
        ...
        imageCaption: needsIllustration ? `A chapter scene for ${visibleTitle}` : undefined,
        imageAlt: needsIllustration ? `Illustration pending for ${visibleTitle}` : undefined,
        visualType: needsIllustration ? "illustration-pending" : undefined,
    ...
    const nextBase: Project = {
      ...
      sourceManifest: result.sourceManifest, sourceAssets: [], sourceReview: undefined,
  ...
  async function importExternalIllustrations(candidates: ExternalIllustrationCandidate[], importIssues: string[], sourceFiles: SourceImageUpload[] = []) {
    ...
    const chapters = project.chapters.map((chapter) => {
      const chapterSlots = slots.filter((item) => item.role === "chapter" && item.chapterId === chapter.id).sort((left, right) => left.imageIndex - right.imageIndex);
      if (!chapterSlots.length) return chapter;
      ...
      const placedPages = assignIllustrationsToReaderPages(textPages, readySlots).map((page, pageIndex): ImportedPage => ({
        ...
        purpose: page.illustrationSlotId ? `Text and ${page.illustrationSlotId.toLowerCase()} illustration` : "Reader text",
        body: page.body,
        illustrationSlotId: page.illustrationSlotId,
        illustrationPlacement: page.illustrationPlacement,
        illustrationAnchorId: page.illustrationAnchorId,
    ...
    // Resolve anchors across a saved chapter while preserving every page frame.
    for (const chapter of chapters) {
      const changed = slots.filter((slot) => slot.chapterId === chapter.id && slot.role === "chapter" && slot.status === "ready" && slot.imageUrl && uploaded.has(slot.id));
      const chapterPages = designerPages.filter((page) => page.chapterId === chapter.id && !page.deleted);
      if (!changed.length || !chapterPages.length) continue;
      const root = document.createElement("div");
      const containers = new Map(chapterPages.map((page) => { const section = document.createElement("section"); section.innerHTML = page.html; root.append(section); return [page.slotId, section]; }));
      placeBookIllustrations(root, changed.map((slot) => {
        const previous = currentSlots.find((item) => item.id === slot.id);
        return { slotId: slot.id, placement: slot.placement === "cover" ? "after-opening" : slot.placement, anchorId: slot.anchorId,
          imageUrl: slot.imageUrl!, imageAlt: readerSafeImageCaption(slot.altText, slot.chapterTitle), imageCaption: readerSafeImageCaption(slot.caption),
          previousUrl: previous?.imageUrl, previousCaption: previous ? readerSafeImageCaption(previous.caption) : undefined };
      }));
  ...
  async function openProject(selected: Project) {
    ...
    if (next.adaptationPlanConfirmed && !needsContextPlan) {
      setEditorWorkspace(isDesignerReady(next) ? "designer" : "workflow");
      setView("editor");
      return;
    }
    setView("analysis");
...
- High detail, clean edges, balanced colour, print-safe contrast, 2048 × 1536 pixels or higher`
  ...
  async function openSourceReview() {
    try {if(editorWorkspace === "designer" && designerStudioRef.current)await designerStudioRef.current.saveWholeBook();setShowSourceReview(true);}
    catch(error){notify(error instanceof Error?error.message:"Save the current layout before source review.");}
  }
  async function approveSourceImage(id:string,variant:"original"|"cleaned") {
    const image=project.sourceManifest?.sourceImages.find(image=>image.id===id);
    if(!image)throw new Error("Source image is not in this book.");
    const asset=project.sourceAssets?.find(a=>a.path===(variant==="original"?image.originalPath:image.cleanedPath));
    const original=project.sourceAssets?.find(a=>a.path===image.originalPath);
    if(!asset||!original)throw new Error("The source comparison files are not available.");
    for(const file of [original,asset]){const check=new Image();check.src=file.url;await check.decode().catch(()=>{throw new Error("A source image could not load. Retry before approving it.");});}
    const ids=new Set(image.placements.map(p=>p.id));
    const slots=project.externalIllustrations?.slots.map(slot=>ids.has(slot.id)?{...slot,imageUrl:asset.url,imageKey:asset.key,status:"ready" as const}:slot);
    const chapters=project.chapters.map(chapter=>{
      const importedPages=chapter.importedPages?.map(page=>page.illustrationSlotId&&ids.has(page.illustrationSlotId)?{...page,imageUrl:asset.url,imageKey:asset.key}:page);
      const first=importedPages?.find(page=>page.imageUrl);
      return {...chapter,importedPages,...(first?{imageUrl:first.imageUrl,imageKey:first.imageKey}:{})};
    });
    const designerPages=project.designerPages?.map(page=>{const root=document.createElement("div");root.innerHTML=page.html;let changed=false;
      root.querySelectorAll<HTMLImageElement>("img[data-illustration-slot]").forEach(node=>{if(ids.has(node.dataset.illustrationSlot!)){node.src=asset.url;changed=true;}});
      return changed?{...page,html:root.innerHTML,savedAt:new Date().toISOString()}:page;
    });
    const sourceReview:SourceBookReview={images:{...project.sourceReview?.images,[id]:{signature:sourceImageSignature(image,project.sourceAssets),variant,approvedAt:new Date().toISOString()}},verses:{...project.sourceReview?.verses}};
    const next={...project,chapters,designerPages,sourceReview,externalIllustrations:project.externalIllustrations?{...project.externalIllustrations,slots:slots!}:undefined};
    await persistProject(next);setProject(next);
  }
  async function approveSourceVerse(id:string,sourceText:string,sourceFile:string,method:"pdf-text"|"manual") {
    const verse=project.sourceManifest?.verses.find(verse=>verse.id===id);
    if(!verse||!compareSourceVerse(verse.sanskrit,sourceText))throw new Error("The source text does not match this Sanskrit verse.");
    const sourceReview:SourceBookReview={images:{...project.sourceReview?.images},verses:{...project.sourceReview?.verses,[id]:{signature:sourceVerseSignature(verse),sourceText,sourceFile,method,approvedAt:new Date().toISOString()}}};
    const next={...project,sourceReview};await persistProject(next);setProject(next);
  }
  ...
  async function createPdf(mode: PdfExportMode) {
    ...
    try {
      ...
      pdf.setProperties({ title: project.title, subject: mode === "draft" ? "Draft book proof — not for publication" : (unfinished ? "Book export — editorial review incomplete" : "Publication book export"), author: "IKS Book Studio", creator: "IKS Book Studio" });
      for (let index = 0; index < sheets.length; index += 1) {
        try {
          const canvas = await renderBookPageCanvas(sheets[index], { backgroundColor: "#fffdf8", scale: raster.scale });
          try {
      ...
      pdf.save(`${safeTitle}-${exportFormat.id}${mode === "draft" ? "-draft-proof" : "-publication"}.pdf`);
      notify(mode === "draft" ? `${exportFormat.label} draft proof downloaded with ${unfinished} publication blocker${unfinished === 1 ? "" : "s"}` : `${exportFormat.label} publication PDF downloaded${unfinished ? ` · ${unfinished} review item${unfinished === 1 ? "" : "s"} remain` : ""}`);
    } catch (reason) {
  ...
  async function downloadPdf() {
    await createPdf("draft");
  }

  async function downloadPublicationPdf() {
    await createPdf("publication");
  }
  ...
  async function openProductionWorkflow(target: "top" | "chapters" | "illustrations" = "top") {
    try {
      if (editorWorkspace === "designer") await designerStudioRef.current?.saveWholeBook();
      setEditorWorkspace("workflow");
      requestAnimationFrame(() => requestAnimationFrame(() => {
        if (target === "chapters") document.querySelector(".chapter-generation-grid")?.scrollIntoView({ behavior: "smooth", block: "start" });
        if (target === "illustrations") {
          document.querySelector<HTMLButtonElement>(".assistant nav button:nth-child(2)")?.click();
  ...
  return (
    <div className="studio-shell">
      ...
      {view === "editor" && editorWorkspace === "workflow" && active && <SimpleWorkflowBar
        project={project}
        active={active}
        busy={draftBusy}
        paused={bookPaused}
      ...
      {showPreview && <CanvaPreview onSourceReview={() => void openSourceReview()} project={project} exportBusy={exportBusy} pdfProgress={pdfProgress} pdfExportMode={pdfExportMode} onClose={() => setShowPreview(false)} onDownload={(mode) => void (mode === "draft" ? downloadPdf() : downloadPublicationPdf())} onUseFormat={async (selectedFormat) => { const pages = resolveBookPages(project); const next = { ...project, bookFormat: selectedFormat, designerPages: pages, designerPageOrder: pages.map((page) => page.slotId), designerLayoutSnapshot: true }; await persistProject(next); notify(`${bookFormat(selectedFormat).label} is now the saved book size`); }} onSaveCanvaPage={uploadCanvaPage} onSetCanvaActive={setCanvaPageActive} />}
      {showSourceReview && <SourceBookReviewPanel project={project} onClose={() => setShowSourceReview(false)} onImage={approveSourceImage} onVerse={approveSourceVerse}/>}
...
function Wizard({ step, project, sourceBusy, sourceProgress, onPatch, onFile, onBack, onNext }: { step: number; project: Project; sourceBusy: boolean; sourceProgress: number; onPatch: (patch: Partial<Project>) => void; onFile: (e: ChangeEvent<HTMLInputElement>) => void; onBack: () => void; onNext: () => void }) {
  return <div className="wizard-layout">
    ...
    <main className="wizard-main">
      ...
      {step === 2 && <SourceBookOptions value={project.sourceBookOptions} onChange={(sourceBookOptions) => onPatch({sourceBookOptions})}/>}
      {step === 3 && <SourceBookOptionsReview value={project.sourceBookOptions}/>}
...
function ExternalIllustrationWorkflow({ project, onBack, onReplaceManuscript, onImport, onOpenDesigner, onNotify }: {
  ...
  const promptPack = useMemo(() => buildExternalIllustrationPromptPack(promptProject), [promptProject]);
  ...
  const downloadPrompt = () => {
    ...
    const anchor = document.createElement("a");
    anchor.href = href;
    anchor.download = `${project.title.replace(/[^a-z0-9]+/gi, "-").replace(/^-|-$/g, "").toLowerCase() || "book"}-sequential-image-prompts.zip`;
    anchor.click();
    URL.revokeObjectURL(href);
  ...
  const copyPrompt = async () => {
    ...
    onNotify("Art bible and placement manifest copied. Use the downloaded prompt pack to generate one image at a time.");
  };
...
function paginateReaderHtml(html: string, audience: string, formatId: BookFormatId = LEGACY_BOOK_FORMAT) {
  ...
  const age = childAgeBand(audience);
  // Keep the established A4 calibration as the baseline, then scale it by the
  // selected format's printable physical area so a size change truly reflows.
  const baseCapacity = age === "7-9" ? 2550 : age === "13-15" ? 3000 : 2750;
  const pageCapacity = Math.max(900, Math.round(baseCapacity * bookFormatCapacityRatio(formatId)));
...
function readerPagesForFormat(chapter: Chapter, audience: string, formatId: BookFormatId) {
  ...
  if (typeof document !== "undefined") {
    ...
    placeBookIllustrations(root, images.map((image, index) => ({
      slotId: image.illustrationSlotId ?? `chapter-${chapter.id}-illustration-${index + 1}`,
      placement: image.illustrationPlacement ?? "chapter-middle", anchorId: image.illustrationAnchorId,
      imageUrl: image.imageUrl!, imageAlt: readerSafeImageCaption(image.imageAlt, `Illustration for ${chapter.title}`),
      imageCaption: readerSafeImageCaption(image.imageCaption),
    })));
    body = root.innerHTML;
  }
...
function Preview({ project, exportBusy, pdfProgress, onClose, onDownload }: { project: Project; exportBusy: boolean; pdfProgress: number; onClose: () => void; onDownload: () => void }) {
  ...
  const chapterSheets = printable.chapters.flatMap((chapter) => {
    ...
    const pages = paginateReaderHtml(chapter.body, project.audience, project.bookFormat);
  ...
  const renderSheet = (sheet: (typeof sheets)[number], key: string) => {
    ...
    if (sheet.kind === "cover") return <article className={`${base} preview-cover`} key={key}><div className="cover-edition">IKS BOOK STUDIO · {project.audience.toUpperCase()}</div><h1>{project.title}</h1><span>{coverSubtitleFor(project)}</span><b>✦</b>{unresolved.length > 0 && <small className="draft-status">Draft proof · {unresolved.length} chapter{unresolved.length === 1 ? "" : "s"} still in progress</small>}</article>;
...
function designerBasePages(project: Project): DesignerBasePage[] {
  ...
  const pages: DesignerBasePage[] = [
    { slotId: "cover", label: "Front cover", kind: "cover", html: `<div class="cover-edition">IKS BOOK STUDIO · ${escapeHtml(project.audience.toUpperCase())}</div><h1>${escapeHtml(project.title)}</h1><span>${escapeHtml(coverSubtitleFor(project))}</span><b>✦</b>` },
...
function designerBookClasses(project: Project, border = project.bookBorder) {
  return `${bookPersonaClass(project.bookPersona)} world-${normalizedTitle(project.aesthetic).replace(/[^a-z]+/g, "-")} page-aesthetic-${normalizedTitle(project.pageAesthetic).replace(/[^a-z]+/g, "-")} book-border-${normalizedTitle(bookBorder(border).value).replace(/[^a-z]+/g, "-")} typography-${normalizedTitle(project.fontTheme).replace(/[^a-z]+/g, "-")}`;
}
...
function defaultDesignerRevision(html = ""): DesignerPageRevision {
  return { html, intentionalBlank: false, deleted: false, layoutLocked: false, backgroundColor: "#fffdf8", watermarkText: "", watermarkOpacity: .12, watermarkRotation: -25, contentVisible: true, watermarkVisible: true, fontFamily: "Georgia, serif", fontSize: 15, textColor: "#243b34", lineHeight: 1.55, letterSpacing: 0, paragraphSpacing: 11, columns: 1, columnGap: 28, pagePadding: 60, borderStyle: "inherit", borderColor: "#b4863e", borderWidth: 2, borderInset: 10, borderRadius: 0, backgroundSize: "cover", backgroundPosition: "center", backgroundOpacity: 1, watermarkRepeat: false, watermarkX: 50, watermarkY: 50, savedAt: new Date().toISOString() };
}

function normalizedDesignerColumns(value: unknown) {
  return Number(value) === 2 ? 2 : 1;
}

function designerPageSupportsColumns(page: { kind: string; chapterId?: number }) {
  return page.kind === "chapter" || (page.kind === "custom" && typeof page.chapterId === "number");
}

function designerColumnsForPage(page: { kind: string; chapterId?: number }, value: unknown) {
  return designerPageSupportsColumns(page) ? normalizedDesignerColumns(value) : 1;
}

function designerColumnVariables(columns: unknown, gap: unknown) {
  const safeColumns = normalizedDesignerColumns(columns);
  const safeGap = Math.max(12, Math.min(60, Number(gap) || 28));
  return {
    "--designer-column-count": String(safeColumns),
    "--designer-column-gap": safeColumns === 1 ? "0px" : `${safeGap}px`,
  } as CSSProperties;
}
...
function hydrateDesignerOverride(page: DesignerPageOverride): DesignerPageOverride {
  const hydrated = hydrateDesignerRevision(page, page.html);
  const columns = designerColumnsForPage(page, hydrated.columns);
  return {
    ...page,
    ...hydrated,
    columns,
    history: (page.history ?? []).map((revision) => ({
      ...hydrateDesignerRevision(revision, revision.html),
      columns: designerColumnsForPage(page, revision.columns),
    })),
  };
}
...
/** The page list used by the editor, reader and PDF capture. A saved book is
 * materialized: changing trim size cannot splice regenerated chapter pages
 * into it. Reflow is an explicit Designer operation on the saved content. */
function resolveBookPages(project: Project): DesignerPageOverride[] {
  const saved = (project.designerPages ?? []).map(hydrateDesignerOverride);
  const generated = designerBasePages(project);
  const base = generated.filter((page) => !project.designerLayoutSnapshot || !saved.some((candidate) =>
    page.kind === "chapter" ? candidate.chapterId === page.chapterId : candidate.kind === page.kind));
  const pages = new Map(base.map((page) => [page.slotId, { ...hydrateDesignerRevision(undefined, page.html), ...page, history: [] } as DesignerPageOverride]));
  saved.forEach((page) => pages.set(page.slotId, page));
  const naturalOrder = [...new Set([...generated.map((page) => page.slotId), ...pages.keys()])].filter((id) => pages.has(id));
  const order = [...new Set((project.designerPageOrder ?? []).filter((id) => pages.has(id)))];
  // Put newly generated pages next to their neighbours, never after the back
  // cover just because the persisted order predates those pages.
  naturalOrder.forEach((id, index) => {
    if (order.includes(id)) return;
    const page = pages.get(id)!;
    const previous = naturalOrder.slice(0, index).reverse().find((candidate) => order.includes(candidate) && pages.get(candidate)?.kind !== "back");
    const beforeBack = order.indexOf("back");
    const insertion = previous ? order.indexOf(previous) + 1 : beforeBack >= 0 ? beforeBack : order.length;
    order.splice(page.kind === "back" ? order.length : insertion, 0, id);
  });
  return order.map((id) => pages.get(id)!);
}

function bookContentStyle(revision: DesignerPageRevision, page: { kind: string; chapterId?: number }): CSSProperties {
...
function renderBookPage(project: Project, page: DesignerPageOverride, key: string, options: {
  draftProof?: boolean;
  editorProps?: React.HTMLAttributes<HTMLDivElement> & { ref: (node: HTMLDivElement | null) => void };
  onMouseDown?: React.MouseEventHandler<HTMLElement>;
} = {}) {
  const canva = project.canvaPages?.find((candidate) => candidate.slotId === page.slotId && candidate.active);
  const background: CSSProperties = { backgroundImage: page.backgroundImageUrl ? `url(${page.backgroundImageUrl})` : undefined,
    backgroundSize: page.backgroundSize === "repeat" ? "auto" : page.backgroundSize,
    backgroundRepeat: page.backgroundSize === "repeat" ? "repeat" : "no-repeat", backgroundPosition: page.backgroundPosition, opacity: page.backgroundOpacity };
  const border: CSSProperties | undefined = page.borderStyle === "none" || page.borderStyle === "inherit" ? undefined : {
  ...
  return <article key={key} data-page-slot={page.slotId} data-book-format={project.bookFormat}
    ...
    {canva ? <img src={canva.current.imageUrl} alt={`${page.label} designed in Canva`}/> : <>
      ...
      <div className={`designer-watermark-layer${page.watermarkRepeat ? " repeat" : ""}`} style={{ opacity: page.watermarkOpacity, left: `${page.watermarkX}%`, top: `${page.watermarkY}%`, transform: `translate(-50%,-50%) rotate(${page.watermarkRotation}deg)`, display: page.watermarkVisible ? "grid" : "none" }}>
        {Array.from({ length: page.watermarkRepeat ? 6 : 1 }, (_, index) => <span key={index}>{page.watermarkImageUrl && <img src={page.watermarkImageUrl} alt=""/>}{page.watermarkText}</span>)}
      </div>
      {!page.intentionalBlank && page.contentVisible && <div className="designer-editable-content" style={bookContentStyle(page, page)}
        {...(options.editorProps ?? { dangerouslySetInnerHTML: { __html: page.html } })}/>}
    </>}
    {options.draftProof && <div className="draft-proof-overlay">{DRAFT_PROOF_LABEL}</div>}
  </article>;
}
...
function designerPageFill(html: string, audience: string, firstPage: boolean, formatId: BookFormatId = LEGACY_BOOK_FORMAT) {
  const age = childAgeBand(audience);
  const capacity = (age === "7-9" ? 2550 : age === "13-15" ? 3000 : 2750) * bookFormatCapacityRatio(formatId) * (firstPage ? .78 : 1);
...
type DesignerStudioHandle = {
  saveWholeBook: () => Promise<Project>;
...
type DesignerStudioProps = {
  ...
  onClose: () => void;
  onPreview: () => void;
  onCommit: (next: Project) => Promise<void>;
};

const DesignerStudio = forwardRef<DesignerStudioHandle, DesignerStudioProps>(function DesignerStudio({ project, embedded = false, onClose, onPreview, onCommit }, ref) {
  const savedPages = (project.designerPages ?? []).map(hydrateDesignerOverride);
  const allPages = useMemo(() => resolveBookPages(project), [project]);
  const orderedPages = allPages;
  const order = orderedPages.map((page) => page.slotId);
  ...
  const [pageMeasurements, setPageMeasurements] = useState<Record<string, ReturnType<typeof measureBookContent>>>({});
  ...
  const [pendingBackground, setPendingBackground] = useState<{
    ...
    targetSlotId: string;
    targetLabel: string;
    backgroundSize: DesignerPageRevision["backgroundSize"];
    backgroundPosition: DesignerPageRevision["backgroundPosition"];
    backgroundOpacity: number;
  } | null>(null);
  ...
  const dismissBackgroundChoice = (nextMessage: string) => {
    ...
    window.requestAnimationFrame(() => backgroundUploadInputRef.current?.focus());
  };

  useEffect(() => {
    if (!pendingBackground) return;
    const focusTimer = window.setTimeout(() => backgroundChoicePrimaryRef.current?.focus(), 0);
    const handleKeyDown = (event: KeyboardEvent) => {
      if (event.key !== "Escape" || busy) return;
      event.preventDefault();
      dismissBackgroundChoice("Background choice cancelled. The current design was not changed.");
    };
  ...
  const commitDesignerProject = async (next: Project) => {
    await onCommit(next);
    const changed = new Set<string>();
    (next.designerPages ?? []).forEach((page) => {
      if (page === savedPages.find((previous) => previous.slotId === page.slotId)) return;
      changed.add(page.slotId);
      liveBookDrafts.current[page.slotId] = page;
      liveBookHtml.current[page.slotId] = page.html;
      const root = bookEditors.current.get(page.slotId);
      if (root && persistableDesignerHtml(root.innerHTML) !== page.html) root.innerHTML = page.html;
      if (page.slotId === selectedId) setDraft(page);
    });
    savedPages.filter((page) => !(next.designerPages ?? []).some((candidate) => candidate.slotId === page.slotId)).forEach((page) => {
      delete liveBookDrafts.current[page.slotId]; delete liveBookHtml.current[page.slotId]; changed.add(page.slotId);
    });
    setDirtyPages((current) => current.filter((id) => !changed.has(id)));
  };
  ...
  const clampFreeImageFrame = (frame: HTMLElement, root: HTMLElement) => {
    if (!frame.classList.contains("designer-free-image")) return;
    const rootStyle = window.getComputedStyle(root);
    const width = parseFloat(frame.style.width) || frame.offsetWidth || 280;
    const height = parseFloat(frame.style.height) || frame.offsetHeight || 180;
    const currentLeft = parseFloat(frame.style.left) || 0;
    const currentTop = parseFloat(frame.style.top) || 0;
    const pageRightInset = Math.max(0, parseFloat(rootStyle.paddingRight) || 0);
    const footerClearance = Math.max(58, parseFloat(rootStyle.paddingBottom) || 0);
    const maxLeft = Math.max(0, root.clientWidth - width - pageRightInset);
    const maxTop = Math.max(0, root.clientHeight - height - footerClearance);
    frame.style.left = `${Math.min(maxLeft, Math.max(0, currentLeft))}px`;
    frame.style.top = `${Math.min(maxTop, Math.max(0, currentTop))}px`;
  };
  ...
  const changeDraft = (patch: Partial<DesignerPageRevision>, remember = true) => {
    if (Object.prototype.hasOwnProperty.call(patch, "columns") && !designerPageSupportsColumns(selected)) return;
    const safePatch = Object.prototype.hasOwnProperty.call(patch, "columns")
      ? { ...patch, columns: designerColumnsForPage(selected, patch.columns) }
      : patch;
  ...
  const pushTextUndo = (slotId: string) => {
    if (slotId !== selectedId) return;
    const snapshot = currentSnapshot();
    setUndoStack((stack) => (stack.at(-1)?.html === snapshot.html ? stack : [...stack.slice(-29), snapshot]));
    setRedoStack([]);
  };
  ...
  const makeOverride = (descriptor: Pick<DesignerPageOverride, "slotId" | "label" | "kind" | "chapterId" | "pageIndex">, revision: DesignerPageRevision, existing?: DesignerPageOverride): DesignerPageOverride => {
    ...
    return { ...revision, html: persistableDesignerHtml(revision.html), slotId: descriptor.slotId, label: descriptor.label, kind: descriptor.kind, chapterId: descriptor.chapterId, pageIndex: descriptor.pageIndex, history: snapshot ? [snapshot as unknown as DesignerPageRevision & { savedAt: string }, ...existing!.history].slice(0, 5) as unknown as DesignerPageRevision[] : [] };
  };
  const saveRevision = async (revisionToSave = draft) => {
    ...
    try {
      ...
      await commitDesignerProject(next); liveBookDrafts.current[selected.slotId] = replacement; setDraft(replacement); setDirtyPages((current) => current.filter((slotId) => slotId !== selected.slotId)); setUndoStack([]); setRedoStack([]); setMessage(`${selected.label} saved. Preview and PDF now use this exact page.`);
    } finally { setBusy(false); }
  };
  ...
  const saveWholeBook = async () => {
    ...
    try {
      ...
      orderedPages.forEach((page) => {
        if (!changed.has(page.slotId) && replacements.has(page.slotId)) return;
      ...
      const next = { ...project, designerPages: [...replacements.values()], designerPageOrder: order, designerLayoutSnapshot: true };
      await commitDesignerProject(next);
      setDirtyPages([]);
  ...
  useImperativeHandle(ref, () => ({ saveWholeBook, previewWholeBook }));

  const flowBodyFromRenderedPage = pageFlowHtml;

  const measureChapterPages = async (chapter: Chapter, flowingHtml: string, styles: DesignerPageRevision[], titleHtml?: string, continuationTitle?: string) => {
    if(flowingHtml.includes("sanskrit-verse"))await document.fonts.load('16px "Book Sanskrit"',"धर्म").catch(()=>undefined);
    await document.fonts?.ready;
    const source = document.createElement("div");
    source.innerHTML = flowingHtml;
    source.querySelectorAll<HTMLElement>("figure").forEach((figure) => {
    ...
    source.querySelectorAll<HTMLImageElement>("figure img").forEach((image) => {
      image.style.setProperty("max-height", `${Math.min(440, bookFormat(project.bookFormat).screenHeightPx * .42)}px`, "important");
      image.style.setProperty("height", "auto", "important"); image.style.minHeight = "0";
      image.style.maxWidth = "100%"; image.style.objectFit = "contain";
    });
    ...
    const liveArticle = original && bookEditors.current.get(original.slotId)?.closest("article");
    if (!liveArticle) throw new Error(`Open Chapter ${chapter.id} in Designer before balancing.`);
    const wrapper = document.createElement("div"); wrapper.className = "book-publication";
    Object.assign(wrapper.style, { position: "fixed", left: "-20000px", top: "0", visibility: "hidden", pointerEvents: "none" });
    const sheet = liveArticle.cloneNode(true) as HTMLElement;
    sheet.removeAttribute("data-page-slot"); sheet.classList.add("designer-measure-page");
    const content = sheet.querySelector<HTMLElement>(".designer-editable-content")!;
    content.removeAttribute("contenteditable");
    wrapper.append(sheet); document.body.append(wrapper);
    try {
      const fits = (pageBlocks: string[], pageIndex: number) => {
        const style = styles[pageIndex] ?? styles.at(-1) ?? defaultDesignerRevision();
        Object.entries(bookContentStyle(style, { kind: "chapter", chapterId: chapter.id })).forEach(([name, value]) => {
          if (name.startsWith("--")) content.style.setProperty(name, String(value));
          else Object.assign(content.style, { [name]: value });
        });
        content.innerHTML = designerChapterPageHtml(project, chapter, pageBlocks.join(""), pageIndex, titleHtml, continuationTitle);
  ...
  const balanceLayout = async (scope: "chapter" | "book", repairIds?: number[]) => {
    const targetChapterIds = repairIds ?? (scope === "book" ? project.chapters.map((chapter) => chapter.id) : selected.chapterId ? [selected.chapterId] : []);
    if (!targetChapterIds.length) { setMessage("Choose a chapter page before balancing this chapter."); return; }
    setBusy(true);
    try {
      const replacements = new Map(orderedPages.map((page) => [page.slotId, { ...page, ...revisionFor(page) }]));
      let nextOrder = [...order];
      let balanced = 0;
      let locked = 0;
      let positioned = 0;
      for (const chapterId of targetChapterIds) {
        ...
        const chapterPages = orderedPages.filter((page) => page.chapterId === chapterId && !revisionFor(page).deleted);
        if (!chapter || !chapterPages.length) continue;
        const revisions = chapterPages.map((page) => ({ page, revision: revisionFor(page) }));
        if (revisions.some(({ revision }) => revision.layoutLocked || revision.intentionalBlank || !revision.contentVisible)) { locked += 1; continue; }
        ...
        // Free placement is a deliberate page design. Preserve it instead of
        // flattening positioned objects into a different reading flow.
        if (/designer-free-image|designer-free-text|designer-text-box|position\s*:\s*(?:absolute|fixed)/i.test(combined) || revisions.some(({ page }) => project.canvaPages?.some((image) => image.slotId === page.slotId && image.active))) { positioned += 1; continue; }
        const firstStyle = revisions[0]?.revision ?? defaultDesignerRevision();
        const firstPage = document.createElement("div"); firstPage.innerHTML = bookEditors.current.get(chapterPages[0].slotId)?.innerHTML ?? firstStyle.html;
        ...
        bodies.forEach((body, pageIndex) => {
          ...
          const descriptor = { ...(existingDescriptor ?? { slotId, kind: "custom" as const, chapterId, html: "" }), label: `Chapter ${chapterId} · page ${pageIndex + 1}`, pageIndex };
      ...
      await commitDesignerProject(next);
      setDirtyPages([]);
      setMessage(`${balanced} chapter${balanced === 1 ? "" : "s"} balanced locally.${locked ? ` ${locked} locked chapter${locked === 1 ? " was" : "s were"} preserved.` : ""} ${positioned ? ` ${positioned} chapter(s) with freely positioned artwork were kept for manual adjustment.` : ""} Text and illustrations were fitted in reading order.`);
    } catch (error) {
      setMessage(error instanceof Error ? `Layout balancing failed: ${error.message}` : "Layout balancing failed.");
    } finally { setBusy(false); }
  };
  useEffect(() => {
    if (automaticLayoutStarted.current || busy || !project.chapters.length) return;
    if (project.chapters.some((chapter) => !(chapter.status === "approved" || chapter.generationStatus === "Completed"))) return;
    automaticLayoutStarted.current = true;
    const firstLayout = !project.designerLayoutSnapshot && !savedPages.some((page) => page.chapterId);
    if (firstLayout) {
      setMessage("Measuring text and illustrations for this book’s first layout…");
      void balanceLayout("book"); return;
    }
    // Upgrade a saved overflowing illustration as soon as the book opens.
    // Reflow itself excludes locked, blank and freely positioned compositions.
    void (async () => {
      await document.fonts?.ready;
      const roots = Array.from(bookEditors.current.entries());
      await Promise.all(roots.flatMap(([, root]) => Array.from(root.querySelectorAll("img")).map((image) => image.decode().catch(() => undefined))));
      if (!roots.length || roots.some(([, root]) => !root.isConnected)) return;
      const repairIds = [...new Set(roots.flatMap(([id, root]) => {
        const page = orderedPages.find((page) => page.slotId === id);
        if (!page?.chapterId || !root.querySelector("figure img")) return [];
        const measured = measureBookContent(root);
        return measured.overflowX || measured.overflowY ? [page.chapterId] : [];
      }))];
      if (repairIds.length) { setMessage("Fitting overflowing illustrations and their captions inside the printable pages…"); await balanceLayout("book", repairIds); }
    })();
  ...
  const uploadBackground = async (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    const targetRevision = revisionFor(target);
    try {
      setBusy(true);
      ...
      setPendingBackground({
        ...
        backgroundSize: targetRevision.backgroundSize,
        backgroundPosition: targetRevision.backgroundPosition,
        backgroundOpacity: targetRevision.backgroundOpacity,
      });
      setMessage(`Background uploaded for ${target.label}. Choose whether to use it on this page or all pages.`);
    } catch (error) {
  ...
  const applyUploadedBackground = async (scope: "page" | "book") => {
    if (!pendingBackground) return;
    const target = descriptorFor(pendingBackground.targetSlotId);
    if (!target) {
      dismissBackgroundChoice("The original page is no longer available. Upload the background again from the page where it should be used.");
      return;
    }
    setBusy(true);
    try {
      const background = {
        ...
        backgroundSize: pendingBackground.backgroundSize,
        backgroundPosition: pendingBackground.backgroundPosition,
        backgroundOpacity: pendingBackground.backgroundOpacity,
      } satisfies Partial<DesignerPageRevision>;
      if (scope === "page") {
        const existing = savedPages.find((page) => page.slotId === target.slotId);
        const current = revisionFor(target);
        const revision = {
          ...current,
          html: bookEditors.current.get(target.slotId)?.innerHTML ?? current.html,
          ...background,
  ...
  const addBlankPage = async () => {
    ...
    await commitDesignerProject({ ...project, designerPages: [...savedPages, page], designerPageOrder: [...order, slotId] }); setSelectedId(slotId);
  };
  const duplicatePage = async () => {
    ...
    await commitDesignerProject({ ...project, designerPages: [...savedPages, page], designerPageOrder: nextOrder }); setSelectedId(slotId);
  };
  const movePage = async (direction: -1 | 1) => {
    ...
    await commitDesignerProject({ ...project, designerPageOrder: nextOrder });
  };
  ...
  const restorePrevious = async () => {
    ...
    await commitDesignerProject({ ...project, designerPages: [...savedPages.filter((page) => page.slotId !== selectedId), replacement] });
  ...
  const resetPage = async () => {
    const base = designerBasePages(project).find((page) => page.slotId === selectedId);
    if (!base) { setMessage("This page has no generated version. Use Restore previous to recover an earlier saved design."); return; }
    const replacement = makeOverride(base, defaultDesignerRevision(base.html), saved);
    await commitDesignerProject({ ...project, designerPages: [...savedPages.filter((page) => page.slotId !== selectedId), replacement], designerPageOrder: order });
    liveBookDrafts.current[selectedId] = replacement;
    replaceEditorHtml(selectedId, replacement.html);
    setDraft(replacement);
    setMessage("Studio page restored in its existing position.");
  };
  const applyStyle = async (scope: "page" | "chapter" | "book") => {
    ...
    const replacements = new Map(savedPages.map((page) => [page.slotId, page]));
    targets.forEach((page) => { const existing = replacements.get(page.slotId); const base = revisionFor(page); replacements.set(page.slotId, makeOverride(page, { ...base, ...style, html: bookEditors.current.get(page.slotId)?.innerHTML ?? base.html, savedAt: new Date().toISOString() }, existing)); });
    await commitDesignerProject({ ...project, designerPages: [...replacements.values()], designerPageOrder: order }); setMessage(`Design applied to ${scope === "book" ? "the whole book" : scope === "chapter" ? "this chapter" : "this page"}.`);
  };
  const savePreset = async () => {
    ...
    await commitDesignerProject({ ...project, designerPresets: [...(project.designerPresets ?? []), preset] }); setMessage(`${preset.name} saved for reuse.`);
  };
  const applyOpacityToBook = async () => {
    const opacity = draft.backgroundOpacity;
    const replacements = new Map(savedPages.map((page) => [page.slotId, page]));
    allPages.forEach((page) => {
      const existing = replacements.get(page.slotId);
      const base = revisionFor(page);
      const updated: DesignerPageRevision = { ...base, backgroundOpacity: opacity, savedAt: new Date().toISOString() };
      replacements.set(page.slotId, makeOverride(page, { ...updated, html: liveBookHtml.current[page.slotId] ?? base.html }, existing));
      if (liveBookDrafts.current[page.slotId]) liveBookDrafts.current[page.slotId] = { ...liveBookDrafts.current[page.slotId]!, backgroundOpacity: opacity };
    });
    // update current draft immediately so slider stays in sync
    setDraft((d) => ({ ...d, backgroundOpacity: opacity }));
    await commitDesignerProject({ ...project, designerPages: [...replacements.values()], designerPageOrder: order });
    setMessage(`Background opacity ${Math.round(opacity * 100)}% applied to all ${allPages.length} pages. Saved — Preview and PDF now use it.`);
  };
  ...
  const wireFreeImageControls = (container: HTMLElement, root: HTMLDivElement, slotId: string, selectImmediately = false) => {
    ...
    const onPointerMove = (event: PointerEvent) => {
      ...
      clampFreeImageFrame(container, root);
    };
  ...
  useEffect(() => {
    ...
    const timer = window.setTimeout(async () => {
      await document.fonts?.ready;
      const roots = orderedPages.filter((page) => !revisionFor(page).deleted).flatMap((page) => {
        const root = bookEditors.current.get(page.slotId); return root ? [{ page, root }] : [];
      });
      await Promise.all(roots.flatMap(({ root }) => Array.from(root.querySelectorAll("img")).map((image) => image.decode().catch(() => undefined))));
      if (!cancelled) setPageMeasurements(Object.fromEntries(roots.map(({ page, root }) => [page.slotId, measureBookContent(root)])));
    }, 160);
    return () => { cancelled = true; window.clearTimeout(timer); };
  }, [project, draft, dirtyPages]);
  const fillForPage = (page: (typeof allPages)[number]) => {
    const measured = pageMeasurements[page.slotId];
    if (!measured) return { ratio: 0, label: "Checking layout", tone: "balanced" };
    const siblings = orderedPages.filter((candidate) => candidate.chapterId === page.chapterId && !revisionFor(candidate).deleted);
    const middle = Boolean(page.chapterId && siblings.length > 2 && page.slotId !== siblings[0]?.slotId && page.slotId !== siblings.at(-1)?.slotId);
    const overflow = measured.overflowX || measured.overflowY;
    const sparse = middle && measured.fillRatio < .55 && !revisionFor(page).intentionalBlank;
    return { ratio: Math.round(measured.fillRatio * 100), label: overflow ? "Overflowing" : sparse ? "Too empty" : "Fits page", tone: overflow ? "overflow" : sparse ? "empty" : "balanced" };
  };
  const runPreflight = () => {
    ...
    if (!draft.intentionalBlank && root) found.push(...inspectBookPage(root));
  ...
  const runBookPreflight = () => {
    ...
    orderedPages.forEach((page) => {
      ...
      if (revision.deleted || revision.intentionalBlank) return;
      const root = bookEditors.current.get(page.slotId);
      const fill = fillForPage(page);
      if (root) inspectBookPage(root).forEach((issue) => found.push({ slotId: page.slotId, label: page.label, issue }));
  ...
  return <div className={embedded ? "designer-embedded-shell" : "modal-backdrop designer-backdrop"}><section className="designer-studio designer-studio-pro book-publication" data-page-watermark={normalizedTitle(project.pageWatermark).replace(/[^a-z]+/g, "-")} style={bookFormatCssVariables(project.bookFormat) as CSSProperties} role={embedded ? "region" : "dialog"} aria-modal={embedded ? undefined : true} aria-label="Designer Studio">
    ...
    <main className="designer-stage"><nav className="designer-toolbar designer-book-toolbar"><button onClick={undo} disabled={!undoStack.length} title="Undo selected page (Cmd+Z)" aria-label="Undo" style={{background: undoStack.length ? "#ffffff" : "#f5f1e8", border: undoStack.length ? "1.5px solid #111111" : "1px solid #ddd4c4", color: undoStack.length ? "#111111" : "#a99e8f", padding:"7px 12px", fontSize:"12px", fontWeight:"800", letterSpacing:".02em", borderRadius:"8px", display:"inline-flex", alignItems:"center", gap:"7px", cursor: undoStack.length ? "pointer" : "not-allowed", opacity: undoStack.length ? 1 : 0.58, boxShadow: undoStack.length ? "0 1px 6px rgba(0,0,0,.11)" : "none", transition:"all 150ms ease"}}><svg width="28" height="18" viewBox="0 0 28 18" fill="none" aria-hidden="true" style={{flexShrink:0, display:"block"}}><path d="M7.2 5.2L2.6 9.1L7.2 13" stroke="currentColor" strokeWidth="3.1" strokeLinecap="round" strokeLinejoin="round"/><path d="M2.6 9.1H16.4C19.9 9.1 23.2 9.1 23.2 13.1C23.2 17.1 19.4 17.1 15.9 17.1H11.2" stroke="currentColor" strokeWidth="3.1" strokeLinecap="round" strokeLinejoin="round"/></svg> Undo</button><button onClick={redo} disabled={!redoStack.length} title="Redo selected page (Shift+Cmd+Z)" aria-label="Redo" style={{background: redoStack.length ? "#ffffff" : "#f5f1e8", border: redoStack.length ? "1.5px solid #111111" : "1px solid #ddd4c4", color: redoStack.length ? "#111111" : "#a99e8f", padding:"7px 12px", fontSize:"12px", fontWeight:"800", letterSpacing:".02em", borderRadius:"8px", display:"inline-flex", alignItems:"center", gap:"7px", cursor: redoStack.length ? "pointer" : "not-allowed", opacity: redoStack.length ? 1 : 0.58, boxShadow: redoStack.length ? "0 1px 6px rgba(0,0,0,.11)" : "none", transition:"all 150ms ease"}}><svg width="28" height="18" viewBox="0 0 28 18" fill="none" aria-hidden="true" style={{flexShrink:0, display:"block"}}><path d="M20.8 5.2L25.4 9.1L20.8 13" stroke="currentColor" strokeWidth="3.1" strokeLinecap="round" strokeLinejoin="round"/><path d="M25.4 9.1H11.6C8.1 9.1 4.8 9.1 4.8 13.1C4.8 17.1 8.6 17.1 12.1 17.1H16.8" stroke="currentColor" strokeWidth="3.1" strokeLinecap="round" strokeLinejoin="round"/></svg> Redo</button><select aria-label="Selected text font" defaultValue="Georgia" onChange={(event) => command("fontName", event.target.value)}><option>Georgia</option><option>Arial</option><option>Verdana</option><option>Trebuchet MS</option><option>Times New Roman</option><option>Noto Serif</option><option>Noto Sans Devanagari</option></select><select aria-label="Selected text size" defaultValue="3" onChange={(event) => command("fontSize", event.target.value)}><option value="1">Small</option><option value="3">Body</option><option value="5">Subheading</option><option value="7">Title</option></select><button onClick={() => command("bold")}><b>B</b></button><button onClick={() => command("italic")}><i>I</i></button><button onClick={() => command("underline")}><u>U</u></button><button onClick={() => command("insertUnorderedList")}>• List</button><button onClick={() => command("justifyLeft")}>Left</button><button onClick={() => command("justifyCenter")}>Centre</button><button onClick={() => command("justifyRight")}>Right</button><label title="Text colour"><input type="color" defaultValue="#243b34" onChange={(event) => command("foreColor", event.target.value)}/></label><label title="Highlight"><input type="color" defaultValue="#f6e6ad" onChange={(event) => command("hiliteColor", event.target.value)}/></label><button onClick={addTextBox}>＋ Text box</button><input ref={imageInputRef} type="file" accept="image/png,image/jpeg,image/webp" style={{display:"none"}} onChange={(e)=>{const f=e.target.files?.[0]; if(f) void addFreeImage(f)}} /><button onClick={()=>imageInputRef.current?.click()} disabled={busy} title="Add image anywhere, drag to move, handles to resize">＋ Image</button><button className="balance-chapter-button" onClick={() => void balanceLayout("chapter")} disabled={!selected.chapterId || busy}>Balance this chapter</button></nav>
      <div ref={designerCanvasRef} className={`designer-canvas-wrap designer-whole-book-canvas book-align-${project.bookAlignment || "left"}`} style={{ "--designer-canvas-scale": canvasScale } as CSSProperties} aria-label="Editable whole book" onScroll={handleCanvasScroll} onWheelCapture={interruptProgrammaticReveal} onTouchMoveCapture={interruptProgrammaticReveal}>{orderedPages.filter((page) => !revisionFor(page).deleted).map((page, index) => { const revision = revisionFor(page); const fill = fillForPage(page); const isDirty = dirtyPages.includes(page.slotId); return <section className={`designer-flow-page${page.slotId === selectedId ? " selected" : ""}`} data-designer-slot-id={page.slotId} id={`designer-flow-${page.slotId}`} key={page.slotId}><div className="designer-flow-page-meta"><span>PAGE {index + 1}</span><b>{page.label}</b><button className={`page-fill-badge ${fill.tone}`} onClick={() => { setSelectedId(page.slotId); setPanel("preflight"); }}>{fill.label} · {fill.ratio}%</button><button className={`page-save-button ${isDirty ? "dirty" : "saved"}`} disabled={busy || !isDirty} onClick={() => void savePageById(page.slotId)}>{busy && page.slotId === selectedId ? "Saving…" : isDirty ? "Save page" : "Saved"}</button></div>{renderBookPage(project, { ...page, ...revision }, page.slotId, {
          ...
          editorProps: {
            ...
            onBeforeInput: () => pushTextUndo(page.slotId),
            onInput: (event) => { rememberLiveHtml(page.slotId, event.currentTarget.innerHTML); captureSelection(); },
            onKeyDown: (event) => handleContentsTitleKeyDown(event, page.slotId),
            onKeyUp: () => captureSelection(), onClick: selectCanvasObject,
          },
        })}<div style={{margin:"6px auto 0",fontSize:"11px",letterSpacing:"0.04em",color:"#1a2e1a",background:"#ffffff",padding:"5px 12px",borderRadius:"999px",border:"1px solid #b4532a",boxShadow:"0 2px 8px rgba(0,0,0,0.08)",fontWeight:"800",width:"fit-content"}}>Book page {index+1} of {order.length} — exactly as in Preview</div>{revision.intentionalBlank && <div className="intentional-blank-label">INTENTIONAL BLANK PAGE</div>}</section>; })}</div>
    </main>
    ...
    }}><div className="designer-background-choice" role="dialog" aria-modal="true" aria-labelledby="background-choice-title" aria-describedby="background-choice-description"><img src={pendingBackground.url} alt="Uploaded background preview"/><div><p className="eyebrow">BACKGROUND READY</p><h3 id="background-choice-title">Where should this background be used?</h3><p id="background-choice-description" className="designer-background-choice-target">Uploaded from <b>{pendingBackground.targetLabel}</b></p><span title={pendingBackground.fileName}>{pendingBackground.fileName}</span><div className="designer-background-choice-actions"><button ref={backgroundChoicePrimaryRef} onClick={() => void applyUploadedBackground("page")} disabled={busy}>This page only</button><button className="primary" onClick={() => void applyUploadedBackground("book")} disabled={busy}>All pages</button></div><button className="designer-choice-cancel" onClick={() => dismissBackgroundChoice("Background choice cancelled. The current design was not changed.")} disabled={busy}>Cancel</button></div></div></div>}
  </section></div>;
});
...
function CanvaPreview({ project: savedProject, onSourceReview, exportBusy, pdfProgress, pdfExportMode, onClose, onDownload, onUseFormat, onSaveCanvaPage, onSetCanvaActive }: {
  project: Project;
  exportBusy: boolean;
  pdfProgress: number;
  pdfExportMode: PdfExportMode;
  onClose: () => void;
  onDownload: (mode: PdfExportMode) => void;
  onSourceReview?: () => void;
  onUseFormat: (formatId: BookFormatId) => Promise<void>;
  ...
  const [previewFormat, setPreviewFormat] = useState<BookFormatId>(savedProject.bookFormat);
  ...
  const [qualityReport, setQualityReport] = useState<BookFormatPreviewResult | null>(null);
  const [sourceRenderIssues,setSourceRenderIssues]=useState<{id:string;message:string}[]>([]);
  const printable = useMemo(() => printableChapters(project.chapters), [project.chapters]);
  ...
  const sheets = useMemo(() => resolveBookPages(project).filter((page) => !page.deleted), [project]);
  type Sheet = DesignerPageOverride;
  const chapterSheets = sheets.filter((page) => typeof page.chapterId === "number");
  const draftProof = exportBusy && pdfExportMode === "draft";
  const slotFor = (sheet: Sheet): Omit<CanvaPageOverride, "active" | "current" | "history"> => ({
    slotId: sheet.slotId, label: sheet.label, kind: sheet.kind === "custom" ? "chapter" : sheet.kind,
    chapterId: sheet.chapterId, pageIndex: sheet.pageIndex,
  });
  const overrideFor = (sheet: Sheet) => project.canvaPages?.find((page) => page.slotId === sheet.slotId);
  const designerFor = (sheet: Sheet) => sheet;
  const renderSheet = (sheet: Sheet, key: string) => renderBookPage(project, sheet, key, { draftProof });
  const capturePage = async (slotId: string) => {
    ...
    const element = candidates.find((candidate) => candidate.closest(".pdf-render-stack")) ?? candidates.find((candidate) => !candidate.closest(".designer-flow-page"));
    if (!element) throw new Error("Page is not visible yet");
    const canvas = await renderBookPageCanvas(element, { backgroundColor: "#fffdf8", scale: 2 });
  ...
  useEffect(() => {
    if (formatChooserOpen) return;
    let cancelled = false;
    setQualityReport(null);
    const inspect = async () => {
      if(project.sourceManifest?.verses.length)await document.fonts.load('16px "Book Sanskrit"',"धर्म").catch(()=>undefined);
      await document.fonts?.ready;
      await new Promise((resolve) => window.setTimeout(resolve, 80));
      if (cancelled) return;
      const format = bookFormat(project.bookFormat);
      const elements = Array.from(document.querySelectorAll<HTMLElement>(".preview-v2 .pdf-render-stack .book-sheet"));
      await Promise.all(elements.flatMap((element) => Array.from(element.querySelectorAll<HTMLImageElement>("img")).map((image) => image.complete ? Promise.resolve() : image.decode().catch(() => undefined))));
      if (cancelled) return;
      const metrics: BookPageMetric[] = sheets.map((sheet, index) => {
        ...
        const measured = content ? measureBookContent(content) : { fillRatio: 0, overflowX: false, overflowY: false };
        const images = Array.from(element?.querySelectorAll<HTMLImageElement>("img") ?? []).filter((image) => !image.closest(".designer-watermark-layer,.designer-render-watermark")).map((image, imageIndex) => {
          const renderedWidthMm = image.clientWidth / Math.max(1, format.screenWidthPx) * format.widthMm;
          return { id: `${slot.slotId}-image-${imageIndex + 1}`, effectiveDpi: renderedWidthMm > 0 ? image.naturalWidth / renderedWidthMm * 25.4 : 0, alt: image.alt };
        });
        const siblings = sheet.chapterId ? chapterSheets.filter((page) => page.chapterId === sheet.chapterId) : [];
        const chapterPageCount = siblings.length || undefined;
        const kind = sheet.chapterId ? "chapter" : sheet.kind;
        return {
          slotId: slot.slotId,
          label: slot.label,
          kind,
      ...
      if (!cancelled) {setQualityReport(assessBookFormatPreview(project.bookFormat, metrics, project.chapters.length));setSourceRenderIssues([...inspectRenderedSourceVerses(elements,project.sourceManifest),...inspectRenderedSourceImages(elements,project)]);}
    };
    void inspect();
    return () => { cancelled = true; };
  }, [previewFormat, savedProject, pdfExportMode, formatChooserOpen]);

  const sourceWarnings=[...sourceReviewIssues(project),...sourceRenderIssues];
  ...
  useEffect(() => { setPageIndex((index) => Math.min(index, Math.max(0, sheets.length - 1))); }, [sheets.length]);
  const current = sheets[Math.min(pageIndex, Math.max(0, sheets.length - 1))];
  const currentChapter = typeof current?.chapterId === "number" ? current.chapterId : current?.kind === "contents" ? -1 : 0;
  const selectedChapterSheets = chapterSheets.filter((sheet) => sheet.chapterId === selectedChapterId);
  const selectedChapter = project.chapters.find((chapter) => chapter.id === selectedChapterId) ?? project.chapters[0];
  const selectedChapterPosition = Math.max(0, project.chapters.findIndex((chapter) => chapter.id === selectedChapterId));
  const goToChapter = (chapterId: number) => { setSelectedChapterId(chapterId); const index = sheets.findIndex((sheet) => sheet.chapterId === chapterId); if (index >= 0) setPageIndex(index); };
...
function LegacyPreview({ project, draftBusy, onFill, onRefresh, onClose, onPrint }: { project: Project; draftBusy: boolean; onFill: () => void; onRefresh: () => void; onClose: () => void; onPrint: () => void }) {
  ...
  const chapterSheets = printable.chapters.flatMap((chapter) => {
    ...
    const textPages = paginateReaderHtml(chapter.body, project.audience, project.bookFormat);
