...
function DesignerStudio({ project, onClose, onPreview, onCommit }: { project: Project; onClose: () => void; onPreview: () => void; onCommit: (next: Project) => Promise<void> }) {
  ...
  const uploadBackground = async (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      const revision = { ...draft, backgroundImageKey: image.key, backgroundImageUrl: image.url };
      changeDraft({ backgroundImageKey: image.key, backgroundImageUrl: image.url });
      await saveRevision(revision);
      setMessage("Background applied and saved. Preview and PDF now use it.");
    } catch (error) {
